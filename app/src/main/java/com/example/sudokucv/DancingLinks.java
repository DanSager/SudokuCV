...
class Sudoku
{
  ...
  */
    ...
    public void read( String[] in ) throws Exception
    {
        int count = 0;
        for( int i = 0; i < N; i++ ) {
            for( int j = 0; j < N; j++ ) {
                int val = Integer.parseInt(in[count]);
                Grid[i][j] = val;
                count++;
            }
        }
    ...
    public String[] toStringArr(int[][] in) {
        String[] retVal = new String[81];
        int count = 0;
        for (int i = 0; i < 9; i++) {
            for (int j = 0; j < 9; j++) {
                String s = Integer.toString(in[i][j]);
                retVal[count] = s;
                count++;
            }
        }
